...
export default function DoctorDetails() {
  ...
  const loadDates = async () => {
    try {
      const response = await axios.get(
        `${process.env.EXPO_PUBLIC_API_URL}/appointment/dates`
      );
  ...
  const loadAvailabilities = async (date: string) => {
    try {
      const response = await axios.get(
        `${process.env.EXPO_PUBLIC_API_URL}/appointment/availability?staffId=${id}&date=${date}`
      );
  ...
  const loadDoctor = async () => {
    try {
      if (locationState.latitude && locationState.longitude) {
        const response = await axios.get(
          `${process.env.EXPO_PUBLIC_API_URL}/staff/details?staffId=${id}&latitude=${locationState.latitude}&longitude=${locationState.longitude}`
        );
        setDoctor(response.data);
      } else {
        const response = await axios.get(
          `${process.env.EXPO_PUBLIC_API_URL}/staff/details?staffId=${id}`
        );
        setDoctor(response.data);
      }
    } catch (error) {}
      ...
      <ScrollView style={common.container}>
        <View style={styles.doctorContainer}>
          ...
          <View style={styles.doctorInfo}>
            ...
            <Text style={common.text}>
              {doctor.experience} years experience • ~{doctor.distance} kms
            </Text>
        ...
        <View style={{ paddingTop: 20 }}>
          <View style={styles.titleContainer}>
            ...
            <Text style={common.title}>Morning Slots</Text>
          </View>
          <View
        ...
        <View style={{ paddingTop: 20 }}>
          <View style={styles.titleContainer}>
            ...
            <Text style={common.title}>Evening Slots</Text>
          </View>
          <View
            style={{ display: "flex", flexDirection: "row", paddingTop: 10 }}
          >
